...
void createVertexBuffer(VkDevice device, VkBuffer vertexBuffer){
...
Pipeline *gf3d_pipeline_graphics_load(VkDevice device,char *vertFile,char *fragFile,VkExtent2D extent)
{
	...
	VkVertexInputAttributeDescription *attributeDescription[2] = {0}; // not sure about this
	...
	if (vkAllocateMemory(device, &allocInfo, NULL, &vertexBufferMemory) != VK_SUCCESS) {
		slog("failed to allocate vertex buffer memory!");
	}

	vkBindBufferMemory(device, vertexBuffer, vertexBufferMemory, 0);

	vertexInputInfo.vertexBindingDescriptionCount = 1;
	vertexInputInfo.vertexAttributeDescriptionCount = (uint32_t)(sizeof(attributeDescription));
...
uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties, VkDevice device) {
